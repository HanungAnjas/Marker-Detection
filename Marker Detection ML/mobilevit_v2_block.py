    ...
    def folding_pytorch(self, patches: torch.Tensor, output_size: Tuple[int, int]) -> torch.Tensor:
        ...
        for i in range(n_patches_h):
            for j in range(n_patches_w):
                ...
                patch = patch.reshape(batch_size, in_dim, self.patch_h, self.patch_w)
                feature_map[:, :, i*self.patch_h:(i+1)*self.patch_h, j*self.patch_w:(j+1)*self.patch_w] = patch
